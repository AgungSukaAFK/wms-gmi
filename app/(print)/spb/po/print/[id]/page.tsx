  ...
  useEffect(() => {
    const run = async () => {
      ...
      const { data: poData } = await supabase
        .from("spb_po")
        .select("*, spb:spb_id(spb_no, spb_gudang)")
        .eq("id", id)
        .single();
      const { data: detailRows } = await supabase
        .from("spb_po_details")
        .select(
          "id, spb_detail:spb_dtl_id(dtl_spb_part_number, dtl_spb_part_name, dtl_spb_qty, dtl_spb_part_satuan)",
        )
        .eq("spb_po_id", id)
        .order("created_at");
      setHeader(poData || null);
      setItems(detailRows || []);
      setLoading(false);
      if (poData) setTimeout(() => window.print(), 600);
    };
  ...
  if (!header)
    return (
      <div className="p-8 text-center">
        <p>Dokumen SPB-PO tidak ditemukan.</p>
      </div>
    );
    ...
    <Content>
      <div className="bg-white min-h-screen p-0 sm:p-8">
        ...
        <div className="max-w-[210mm] mx-auto bg-white p-[15mm]">
          <h1 className="text-2xl font-bold">PURCHASE ORDER (SPB)</h1>
          <p className="text-sm text-muted-foreground">No PO: {header.po_no}</p>
